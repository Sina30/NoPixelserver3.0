...
function removePlayerFromRadio(source, currentChannel)
	...
	for player, _ in pairs(radioData[currentChannel]) do
		TriggerClientEvent('ab-voice:removePlayerFromRadio', player, source)
	end
...
function addPlayerToRadio(source, channel)
	...
	for player, _ in pairs(radioData[channel]) do
		TriggerClientEvent('ab-voice:addPlayerToRadio', player, source)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)

	voiceData[source].radio = channel
	radioData[channel][source] = false
	TriggerClientEvent('ab-voice:syncRadioData', source, radioData[channel])
end
...
function setPlayerRadio(source, radioChannel)
	...
	if GetInvokingResource() then
		...
		-- changed
		TriggerClientEvent('ab-voice:clSetPlayerRadio', source, radioChannel)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('ab-voice:setPlayerRadio', function(radioChannel)
	...
	if radioTbl then
		for player, _ in pairs(radioTbl) do
			if player ~= source then
				TriggerClientEvent('ab-voice:setTalkingOnRadio', player, source, talking)
			end
		end
	end
end
