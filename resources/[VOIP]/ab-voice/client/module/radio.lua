...
RegisterNetEvent('ab-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('ab-voice:setTalkingOnRadio', setTalkingOnRadio)
...
RegisterNetEvent('ab-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('ab-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('ab-voice:setPlayerRadio', channel)
...
RegisterCommand('+radiotalk', function()
    ...
    if not voiceData.radioPressed and voiceData.radioEnabled then
        if voiceData.radio > 0 then
            TriggerServerEvent('ab-voice:setTalkingOnRadio', true)
            voiceData.radioPressed = true
			playMicClicks(true)
            Citizen.CreateThread(function()
                TriggerEvent("ab-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	if voiceData.radio > 0 or voiceData.radioEnabled then
		...
		TriggerEvent("ab-voice:radioActive", false)
		playMicClicks(false)
		TriggerServerEvent('ab-voice:setTalkingOnRadio', false)
...
RegisterNetEvent('ab-voice:clSetPlayerRadio', syncRadio)
