	...
	Citizen.CreateThread(function()
		...
		while voiceData.call ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				changed = true
				TriggerServerEvent('ab-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				TriggerServerEvent('ab-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('ab-voice:syncCallData')
AddEventHandler('ab-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('ab-voice:setTalkingOnCall')
AddEventHandler('ab-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('ab-voice:addPlayerToCall')
AddEventHandler('ab-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
	playerTargets(radioData, callData)
end)

RegisterNetEvent('ab-voice:removePlayerFromCall')
AddEventHandler('ab-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('ab-voice:setPlayerCall', channel)
...
RegisterNetEvent('ab-voice:clSetPlayerCall')
AddEventHandler('ab-voice:clSetPlayerCall', function(callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
