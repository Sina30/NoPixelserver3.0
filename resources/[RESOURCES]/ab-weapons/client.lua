...
RegisterNetEvent('ab-weapons:destroyGroundWeapon')
AddEventHandler('ab-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('ab-weapons:allowDrop')
AddEventHandler('ab-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('ab-weapons:dropweapon')
AddEventHandler('ab-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("ab-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('ab-weapons:doHandOver')
AddEventHandler('ab-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('ab-weapons:dropWeaponSearch')
AddEventHandler('ab-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('ab-weapons:giveweapon')
AddEventHandler('ab-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("ab-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["ab-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
