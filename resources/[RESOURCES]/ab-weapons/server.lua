RegisterNetEvent('ab-weapons:getAmmo')
AddEventHandler('ab-weapons:getAmmo', function()
    local ammoTable = {}
    local src = source
    ...
    exports.ghmattimysql:execute("SELECT type, ammo FROM characters_weapons WHERE id = @id", {['id'] = char.id}, function(result)
        ...
        TriggerClientEvent('ab-items:SetAmmo', src, ammoTable)
    end)
end)

RegisterNetEvent('ab-weapons:updateAmmo')
AddEventHandler('ab-weapons:updateAmmo', function(newammo,ammoType,ammoTable)
    local src = source
	local user = exports["ab-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
