...
local function disconnect()
    TriggerServerEvent("ab-login:disconnectPlayer")
end
...
local function nuiCallBack(data)
    ...
    if data.newchar then
        ...
        events:Trigger("ab-base:createCharacter", data.chardata, function(created)
            if not created then
                created = {
                    err = true,
    ...
    if data.deletecharacter then
        ...
        events:Trigger("ab-base:deleteCharacter", data.deletecharacter, function(deleted)
            sendMessage({reload = true})
        end)
    end

    if data.selectcharacter then
        events:Trigger("ab-base:selectCharacter", data.selectcharacter, function(data)
