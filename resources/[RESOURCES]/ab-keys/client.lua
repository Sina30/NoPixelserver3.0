...
AddEventHandler('keys:give', function()
  ...
  if IsPedInAnyVehicle(PlayerPedId(), false) and hasKey(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId()))) then

    t, distance = GetClosestPlayer()

    if(distance ~= -1 and distance < 5) then
      TriggerServerEvent('keys:send', GetPlayerServerId(t), GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())))
    ...
    TriggerEvent("DoLongHudText", "You dont have keys for this vehicle!",2)
    return
  end
...
function shutoffenginesearch()
    ...
    if not fuckingRETARDED then
       ...
       fuckingRETARDED = true
        ...
        local finished = exports["ab-taskbar"]:taskBar(5000,"Searching")
        Citizen.Wait(100)
        ...
        if luck == 69 and finished == 100 then
          local finished = exports["ab-taskbar"]:taskBar(2000,"Found and Using Keys")
        ...
        local finished = exports["ab-taskbar"]:taskBar(5000,"Searching Backseat")
        if luck > 75 and finished then
          if not IsPedInAnyVehicle(PlayerPedId(), false) then
            fuckingRETARDED = false
            return
          end
          local finished = exports["ab-taskbar"]:taskBar(2000,"Found Content")
          DropItemPed()
        end
...
function shutoffenginehotwire()
    ...
    if not fuckingRETARDED then
        ...
        local finished = exports["ab-taskbar"]:taskBar(carTimer,"Attempting Hotwire")
        Citizen.Wait(100)
          ...
          if not hasKey(plate1) then
            ...
            if pedDriver ~= 0 and (not IsPedAPlayer(pedDriver) or IsEntityDead(pedDriver)) then
              ...
              if IsEntityDead(pedDriver) then
                ...
                local finished = exports["ab-taskbar"]:taskBar(3000,"Taking Keys",false)
                ...
                if GetEntityModel(curveh) ~= `taxi` then
                  ...
                  if math.random(100) > 95 then
                      ...
                      TriggerEvent("civilian:alertPolice",20.0,"lockpick",targetVehicle)
                      local finished = exports["ab-taskbar"]:taskBar(3000,"Taking Keys")
                      if finished == 100 then
                       -- SetEntityAsMissionEntity(curveh,false,true)
                        TriggerEvent("keys:addNew",curveh,plate1)
                      else
    ...
    if aiming then
     if DoesEntityExist(targetPed) and not IsPedAPlayer(targetPed) and IsPedArmed(PlayerPedId(), 7) then
      ...
      if not IsEntityDead(targetPed) and distance < 12 then
       ...
       TriggerEvent("keys:addNew",vehicle,plate1)
       Citizen.Wait(500)
