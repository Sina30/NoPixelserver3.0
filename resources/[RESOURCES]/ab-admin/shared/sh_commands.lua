...
function cmd.RunCommand(caller, args)
    ...
    if not ABX.Admin:CanTargetRank(caller, args.rank) then ABX.Admin:Log(string.format("%s [%s] attempted to set %s's [%s] rank to %s; Admin can't target that rank", caller:getVar("name"), caller:getVar("steamid"), args.target:getVar("name"), args.target:getVar("steamid"), args.rank), caller) return end

    ABX.Admin:SetRank(args.target, args.rank)
...
function cmd.DrawCommand()
        ...
        if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then ABX.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end) end           

        if WarMenu.Button("Select a rank", "Selected: " .. (cmd.vars.rank ~= nil and tostring(cmd.vars.rank) or "None")) then ABX.Admin.Menu:DrawRanks(cmd.command, function(_rank)
            cmd.vars.rank = _rank
        end) end
        ...
        if args.target and args.rank then if WarMenu.Button("Set " .. args.target.name .. "'s rank to " .. args.rank) then ABX.Admin:GetCommandData(cmd.command).runcommand(args) cmd.vars.target = nil cmd.vars.rank = nil end end
end

ABX.Admin:AddCommand(cmd)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("God Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunClCommand(args)
    ABX.Admin.teleportMarker()
end

function cmd.DrawCommand()      
    if WarMenu.Button("Teleport to marker") then ABX.Admin:GetCommandData(cmd.command).runcommand({}) end
end
...
function cmd.DrawCommand()      
    ...
    if WarMenu.Button("Enter x", "x: " .. (cmd.vars.x and cmd.vars.x or "0.0")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Item", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            if string.find(result, ",") then
                local resultSplit = ABX.Admin.split(result, ',')
    ...
    if WarMenu.Button("Enter y", "y: " .. (cmd.vars.y and cmd.vars.y or "0.0")) then
        ABX.Admin.Menu:ShowTextEntry("Enter y", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter z", "z: " .. (cmd.vars.z and cmd.vars.z or "0.0")) then
        ABX.Admin.Menu:ShowTextEntry("Enter z", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.x and cmd.vars.y and cmd.vars.z then if WarMenu.Button("Teleport to: " .. "X:["..cmd.vars.x.."] Y:["..cmd.vars.y.."] Z:["..cmd.vars.z.."]") then ABX.Admin:GetCommandData(cmd.command).runcommand({x = cmd.vars.x,y = cmd.vars.y,z = cmd.vars.z}) end end
end
...
function cmd.DrawCommand()
    cmd.vars.pos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)      
    if WarMenu.Button("Set Dev Spawn") then ABX.Admin:GetCommandData(cmd.command).runcommand({pos = cmd.vars.pos}) end
end
...
function cmd.Init()
    ...
    RegisterNetEvent("ab-admin:NoclipState")
    AddEventHandler("ab-admin:NoclipState", function(toggle)
...
function cmd.RunClCommand(args)
    cmd.vars.enable = args.toggle
    ABX.Admin:RunNclp(cmd.vars.enable)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Noclip: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Teleport to " .. cmd.vars.target.name .. "'s position") then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
    if cmd.vars.lastPos then if WarMenu.Button("Return to your last position") then ABX.Admin:GetCommandData(cmd.command).runcommand({target = false, retn = true}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    ABX.Admin:Bring(caller,args.target.source)
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Bring " .. cmd.vars.target.name .. " to you.") then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
end
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Attach: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a reason", "Reason: " .. (cmd.vars.reason and cmd.vars.reason or "No Reason Given")) then
        ABX.Admin.Menu:DrawTextInput(cmd.vars.reason and cmd.vars.reason or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.target then if WarMenu.Button("Kick " .. cmd.vars.target.name) then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerClientEvent("ab-admin:Cloak", -1, src, args.toggle)
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("ab-base:characterLoaded", function(user, char)
            ...
            TriggerClientEvent("ab-admin:Cloak", src, cmd.vars.cloaked)
        end)

        AddEventHandler("playerDropped", function()
            local src = source
            if cmd.vars.cloaked[src] then
                TriggerClientEvent("ab-admin:Cloak", -1, src, false)
    ...
    RegisterNetEvent("ab-admin:CloakRemote")
    AddEventHandler("ab-admin:CloakRemote", function()
        if ABX.Admin:GetPlayerRank() == "dev" then
            cmd.vars.toggle = not cmd.vars.toggle
            ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle})
        end
    end)

    RegisterNetEvent("ab-admin:Cloak")
    AddEventHandler("ab-admin:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Cloak: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    local temp, timeSum, addedTime = ABX.Admin:GetBanTimeFromString(args.time)

    if temp and temp ~= 0 and timeSum <= 0 then ABX.Admin:Log(string.format("%s [%s] attempted to ban %s [%s]; Invalid ban length", caller:getVar("name"), caller:getVar("steamid"), args.target:getVar("name"), args.target:getVar("steamid"))) return end
    ...
    ABX.Admin:BanPlayer(args.target, caller, args.time, args.reason)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a ban length", "Length: " .. (cmd.vars.time and cmd.vars.time or "No Time Given")) then
        ABX.Admin.Menu:DrawTextInput(cmd.vars.time and cmd.vars.time or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end

                local timeTable, timeSum, addedTime = ABX.Admin:GetBanTimeFromString(result)
    ...
    if cmd.vars.target and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.target.name) then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.target = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.RunCommand(caller, args)
    if not args.steamid or not exports["ab-base"]:getModule("Util"):IsSteamId(args.steamid) then return end
    if not args.reason then args.reason = "No Reason Given" end
    if not args.time then return end

    local players = exports["ab-base"]:getModule("Player"):GetUsers()

    for k,v in pairs(players) do
        local user = exports["ab-base"]:getModule("Player"):GetUser(v)
        if user:getVar("steamid") == args.steamid then ABX.Admin:GetCommandData("ban").runcommand(caller, {target = user, reason = args.reason, time = args.time}) return end
    end

    local temp, timeSum, addedTime = ABX.Admin:GetBanTimeFromString(args.time)

    if temp and temp ~= 0 and timeSum <= 0 then ABX.Admin:Log(string.format("%s [%s] attempted to ban steamid: %s; Invalid ban length", caller:getVar("name"), caller:getVar("steamid"), args.steamid)) return end
    ...
    ABX.Admin:Log(log, caller)
    ABX.Admin:BanId(args.steamid, caller, args.time, args.reason)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a Steam ID", "Entered: " .. (cmd.vars.steamid and cmd.vars.steamid or "None")) then 
        ABX.Admin.Menu:ShowTextEntry("Enter a Steam ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
                if not exports["ab-base"]:getModule("Util"):IsSteamId(result) then result = nil end
            end
    ...
    if cmd.vars.steamid and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.steamid) then ABX.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.steamid = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.RunCommand(caller, args)
    ...
    ABX.Admin.DB:UnbanSteamId(args.steamid)
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.steamid then if WarMenu.Button("Unban " .. cmd.vars.steamid) then ABX.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid}) cmd.vars.steamid = nil end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Tempban " .. cmd.vars.target.name) then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target}) cmd.vars.target = nil end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a message", "Message: " .. (cmd.vars.message and cmd.vars.message or "No Message")) then
        ABX.Admin.Menu:ShowTextEntry("Enter a Message", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Send Message: " .. cmd.vars.message) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    if not args.message then return end
    local src = caller:getVar("source")
    TriggerClientEvent("ab-admin:runSpawnCommand",src,args.message)
    exports["ab-log"]:AddLog("Admin", caller, "Spawned car", {car = tostring(args.message)}) 
    ABX.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a model", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        ABX.Admin.Menu:ShowTextEntry("Enter a model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Seat into Vehicle") then TriggerEvent("ab-admin:SeatIntoLast") end

    if cmd.vars.message then if WarMenu.Button("Spawn model: " .. cmd.vars.message) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    ABX.Admin.DB:giveCar(args.message,args.reciver)
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a cid", "Cid: " .. ( cmd.vars.reciver and  cmd.vars.reciver or "No cid")) then
        ABX.Admin.Menu:ShowTextEntry("Enter a cid", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message and cmd.vars.reciver then if WarMenu.Button("Give model: " .. cmd.vars.message.." to "..cmd.vars.reciver) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,reciver = cmd.vars.reciver}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    if (args.message == "badlsdtab" or args.message == "lsdtab") then
        ...
        if steamId ~= "STEAM_0:1:8992379" then
            exports["ab-log"]:AddLog("Admin", caller, "Unauthorized spawn item", {item = tostring(args.message),amount = tostring(args.amount)}) 
            return
        end
    end
    TriggerClientEvent('player:receiveItem', src, args.message, args.amount)
    exports["ab-log"]:AddLog("Admin", caller, "Spawned item", {item = tostring(args.message),amount = tostring(args.amount)}) 
    ABX.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter Amount", "Amount: " .. (cmd.vars.amount and cmd.vars.amount or "1")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Amount", "", function(result)
            print(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message.." | "..cmd.vars.amount) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,amount = cmd.vars.amount}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    exports["ab-log"]:AddLog("Admin", caller, "Changed weather/time/Cycle", {time = args.time, weather = args.weather, light = args.light, blackout = args.blackout}) 
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a time", "Time: " .. (cmd.vars.time and cmd.vars.time or "0")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Time", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Weather status", "Weather: " .. (cmd.vars.weather and cmd.vars.weather or "none")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Weather status", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter a Cycle Effect", "Cycle: " .. (cmd.vars.lightType and cmd.vars.lightType or "none")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Time Cycle Effect", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.time or cmd.vars.weather or cmd.vars.lightType then if WarMenu.Button("Update Server/Client") then ABX.Admin:GetCommandData(cmd.command).runcommand({time = cmd.vars.time, weather = cmd.vars.weather, light = cmd.vars.lightType, blackout = cmd.vars.toggle}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    exports["ab-log"]:AddLog("Admin", caller, "Set Model", {item = tostring(args.message)}) 
    ABX.Admin:Log(log, caller)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter model name", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        ABX.Admin.Menu:ShowTextEntry("Enter Model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Set Model: " .. cmd.vars.message) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then if WarMenu.Button("Send Announcement: " .. cmd.vars.message) then ABX.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.Init()
    if IsDuplicityVersion() then
        ...
        RegisterNetEvent("ab-admin:RaveMode")
        AddEventHandler("ab-admin:RaveMode", function(enabled)
            cmd.vars.toggle = enabled
            TriggerClientEvent("ab-admin:RaveMode", -1, enabled)
        end)

        AddEventHandler("ab-base:characterLoaded", function(user, char)
            local src = user:getVar("source")
            if cmd.vars.toggle then TriggerClientEvent("ab-admin:RaveMode", src, cmd.vars.toggle) end
        end)
    ...
    RegisterNetEvent("ab-admin:RaveMode")
    AddEventHandler("ab-admin:RaveMode", function(toggle)
        cmd.vars.toggle = toggle
...
function cmd.RunClCommand(args)
    TriggerServerEvent("ab-admin:RaveMode", cmd.vars.toggle)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Rave Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Player Blips: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("ab-admin:ReviveInDistance")
    end


    if cmd.vars.target then if WarMenu.Button("Revive and Heal " .. cmd.vars.target.name) then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Fix " .. cmd.vars.target.name .. "'s vehicle") then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
    if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then if WarMenu.Button("Fix Current vehicle.") then ABX.Admin:GetCommandData(cmd.command).runcommand({runontarget = false}) end end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Select a job", "Selected: " .. (cmd.vars.target and cmd.vars.target or "None")) then ABX.Admin.Menu:DrawJobs(cmd.command, function(_target)
        cmd.vars.target = _target
    end) end

    if cmd.vars.target then if WarMenu.Button("Request [" .. cmd.vars.target .. "]") then ABX.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target}) end end
end
...
function cmd.Init()
    ...
    ABX.Admin.Menu:SetSubMenuProperties("filtermenu")
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Entity View: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle ABX.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle, filter = cmd.vars.filter}) end

    if WarMenu.Button("Enter view distance", "Entered: " .. (cmd.vars.distance or "150")) then 
        ABX.Admin.Menu:ShowTextEntry("Enter a distance", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
        ...
        Citizen.CreateThread(function()
            while WarMenu.IsMenuOpened("targetmenu") do
                ...
                for f, t in pairs(cmd.vars.filters) do
                    if WarMenu.MenuButton(f, "command") then cmd.vars.filter = f ABX.Admin.Menu:DrawCommand(cmd.command) return end
                end
                if WarMenu.MenuButton("Back", "command") then ABX.Admin.Menu:DrawCommand(cmd.command) return end
            end
        end)
    end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter an entity ID") then 
        ABX.Admin.Menu:ShowTextEntry("Enter an entity ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
                result = tonumber(result)
                if not result then result = nil end
    ...
    if cmd.vars.ent then if WarMenu.Button("Delete Entity", "Entity: " .. (cmd.vars.ent or "none")) then
        ABX.Admin:GetCommandData(cmd.command).runcommand({ent = cmd.vars.ent})
    end end
end
...
--[[
    function cmd.ChatCommand(args)
        ...
        if not ABX.Admin:RankExists(rank) then return end
        if not ABX.Admin:IsValidUser(target) then return end

        local args = {
            target = exports["ab-base"]:getModule("Player"):getUser(target),
            rank = rank
        }
