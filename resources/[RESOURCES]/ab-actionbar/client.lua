...
AddEventHandler("ab-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
RegisterNetEvent('ab-items:SetAmmo')
AddEventHandler('ab-items:SetAmmo', function(sentammoTable)
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["ab-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["ab-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
function updateAmmoClient()
	...
	TriggerEvent("ab-items:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("ab-weapons:updateAmmo",newammo,ammoType,ammoTable)
...
RegisterNetEvent('ab-item:CheckClientAmmo')
AddEventHandler('ab-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
