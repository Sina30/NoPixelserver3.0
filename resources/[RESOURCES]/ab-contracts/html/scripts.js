$(document).ready(function(){
  ...
  $(".btnPrev").click(function(){
      $.post('http://ab-contracts/previousID', JSON.stringify({}));
  });

  $(".btnNext").click(function(){
      $.post('http://ab-contracts/nextID', JSON.stringify({}));
  });

  $(".btnPay").click(function(){
      $.post('http://ab-contracts/payID', JSON.stringify({}));
  });

  $(".btnGive").click(function(){
      $.post('http://ab-contracts/giveID', JSON.stringify({ target: $("#contractID2").val(), conamount: $("#contractAmount2").val(), coninformation: $("#contractInfo2").val()}));
  });
  ...
  document.onkeyup = function (data) {
    if ( data.which == 27 ) {
      $.post('http://ab-contracts/close', JSON.stringify({}));
    }
    if ( data.which == 37 ) {
      $.post('http://ab-contracts/previousID', JSON.stringify({}));
    }
