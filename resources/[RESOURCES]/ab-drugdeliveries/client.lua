...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["ab-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
				...
				if exports["ab-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     
					-- RepPlus()
					TriggerEvent("inventory:removeItem","inkedmoneybag", 1)   
					cashPayment = cashPayment + 85000  

		        elseif exports["ab-inventory"]:hasEnoughOfItem("rollcash",30,false) then     
		        ...
		        elseif exports["ab-inventory"]:hasEnoughOfItem("markedbills",20,false) then     
		            TriggerEvent("inventory:removeItem","markedbills", 20)   
		            cashPayment = cashPayment + markedbillsprice            
		            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["ab-inventory"]:hasEnoughOfItem("band",20,false) then     
    ...
    while timer > 0 do
        ...
        if not searching then
            ...
            if IsControlJustReleased(1,38) and #(pos - GetEntityCoords(PlayerPedId())) < 3.0 then
                ...
                local finished = exports["ab-taskbar"]:taskBar(15000,"Searching Thug")
...
function CompleteScrapping(vehicle, originalVehicleLocation)
	...
	local finished = exports["ab-taskbar"]:taskBar(20000,"Scrapping Car")
...
AddEventHandler('goldtrade', function()
	if exports["ab-inventory"]:hasEnoughOfItem("goldbar",70,true) then
	  ...
	  --  local dropOff3 = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(chopinfo[3]["x"],chopinfo[3]["y"],chopinfo[3]["z"]))
		...
		if GoldBars < 1.5 then
			...
			if GoldBarTime then
				...
				if IsControlJustReleased(0,38) then
					if exports["ab-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["ab-taskbar"]:taskBar(120000,"Trading Bars")
						if finished == 100 then
