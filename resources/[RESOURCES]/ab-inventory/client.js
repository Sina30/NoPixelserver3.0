...
RegisterNetEvent('ab-base:playerSpawned')
on('ab-base:playerSpawned', (broughtData) => {
...
on('__cfx_nui:dropIncorrectItems', (data, cb) => {
	...
	//  $.post("http://ab-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('ab-items:SetAmmo', (sentammoTable) => {
	if (sentammoTable) {
