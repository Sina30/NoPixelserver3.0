...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://ab-inventory/UpdateSettings',
            JSON.stringify({
...
function invStack(
    ...
    $.post('https://ab-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://ab-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://ab-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://ab-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://ab-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://ab-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://ab-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://ab-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://ab-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://ab-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://ab-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://ab-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://ab-inventory/invuse', JSON.stringify(arr));
