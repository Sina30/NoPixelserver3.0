ABX.DataControls = ABX.DataControls or {}
ABX.Controls = ABX.Controls or {}
ABX.Controls.EventHolder = {}


ABX.Controls.Current = {}
-- Current bind name and keys
ABX.Controls.Default = {
  ["tokoptt"] = "CAPS",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "G",
  ["tokoToggle"] = "LEFTCTRL",
...
ABX.Controls.secondaryBinds = {}

ABX.Controls.Distcheck = {}
ABX.Controls.Distcheck.use = {
	-- create fake id
...
ABX.Controls.Distcheck.useSecondary = {
...
ABX.Controls.Distcheck.useThird = {
	-- hospital garage
...
function ABX.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(ABX.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function ABX.DataControls.encodeSetBindTable(self, bindTable)
	...
	ABX.DataControls.setBindTable(controlTable,true)
end

function ABX.DataControls.toUpper(table)
...
function ABX.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		ABX.Controls.Current  = ABX.DataControls.toUpper(ABX.Controls.Default) 
		ABX.DataControls.setSecondaryBindTable(ABX.Controls.Current)
		TriggerServerEvent('ab-base:sv:player_control_set',ABX.Controls.Current)
		ABX.DataControls.checkForMissing()
	else
		if shouldSend then 
			ABX.Controls.Current = ABX.DataControls.toUpper(controlTable)
			ABX.DataControls.setSecondaryBindTable(ABX.Controls.Current)
			TriggerServerEvent('ab-base:sv:player_control_set',ABX.Controls.Current)
			ABX.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",ABX.DataControls.getTableInKeyNumbers())
end

function ABX.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#ABX.Controls.secondaryBinds  do
				if ABX.Controls.secondaryBinds[i] and ABX.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				ABX.Controls.secondaryBinds[i] = {}
				ABX.Controls.secondaryBinds[i][1] = control1
				ABX.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function ABX.DataControls.getTableInKeyNumbers()
...
function ABX.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(ABX.Controls.Current) do
		if k == bindID then
			found = v
...
function ABX.DataControls.distanceCall(distcheckName)
	local distanceTable = ABX.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if ABX.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-ABX.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			ABX.Controls.EventHolder[key] = GetGameTimer();
...
function ABX.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(ABX.Controls.Default) do
		if ABX.Controls.Current[j] == nil then
			isMissing = true
			ABX.Controls.Current[j] = h
		end
	end
...
function ABX.DataControls.validEvent(event,id,eventID)
	local v = ABX.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if ABX.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-ABX.Controls.EventHolder[eventKey])
	end

	if ABX.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			ABX.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if ABX.Controls.Current["tokoptt"] then
			for i=0,#ABX.Controls.events["general"] do
				local v = ABX.Controls.events["general"][i]
				local keyString = ABX.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#ABX.Controls.secondaryBinds do
							if IsControlPressed(1,ABX.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,ABX.Controls.secondaryBinds[i][1]) then
								isConflicted = ABX.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								ABX.DataControls.validEvent(eventBind,idEvent,i)
							else
								ABX.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#ABX.Controls.secondaryBinds do
									if IsControlPressed(1,ABX.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,ABX.Controls.secondaryBinds[i][2]) then
										isConflicted = ABX.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if ABX.Controls.Current["tokoptt"] then
			for i=0,#ABX.Controls.events["general-dist"] do
				local v = ABX.Controls.events["general-dist"][i]
				local key = Keys[ABX.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					ABX.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("ab-base:cl:player_control")
AddEventHandler("ab-base:cl:player_control", function(controlTable)
	ABX.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('ab-base:setcontrols')
AddEventHandler('ab-base:setcontrols', function()
	TriggerServerEvent('ab-base:sv:player_controls')
end)
