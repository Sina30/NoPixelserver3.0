local stashes = {}
local instash = 0
RegisterNetEvent('ab-stash:setInitialState')
AddEventHandler('ab-stash:setInitialState', function(stashesp)
    print('loaded')
    stashes = stashesp
...
CreateThread(function()
    TriggerServerEvent("ab-stash:fetchInitialState")
end)
...
RegisterNetEvent('ab-stash:getnearstash')
AddEventHandler('ab-stash:getnearstash', function()
    local closestDoorDistance, closestDoorId = 9999.9, -1
    local currentPos = GetEntityCoords(PlayerPedId())
    for id, handle in pairs(stashes) do
        local currentDoorDistance = #(stashes[id].StashEntry - currentPos)
...
RegisterNUICallback('complete', function(data, cb)
    ...
    cb('ok');
    TriggerEvent('ab-stash:getnearstash')
end)
