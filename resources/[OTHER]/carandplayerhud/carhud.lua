...
Citizen.CreateThread( function()
	while true do
		...
		if IsPedShooting(ped) and not IsPedCurrentWeaponSilenced(ped) then
			Citizen.Wait(50)
			TriggerEvent("client:newStress",true,math.random(150,400))
			Citizen.Wait(800)
...
AddEventHandler("ab-jobmanager:playerBecameJob", function(job, name)
...
AddEventHandler("stress:timed",function(alteredValue,scenario)
	...
	TriggerEvent("DoShortHudText",'You feel little relaxed',6)
end)
...
local opacityBars = 0 
local Addition = 0.0
-- RegisterCommand('stressadd', function()
-- 	TriggerServerEvent("server:alterStress", true, 350)
-- end)
...
Citizen.CreateThread( function()
	while true do
		...
		local ped = PlayerPedId()
		local name = GetSelectedPedWeapon(Attackerped)
		if IsPedShooting(ped) and not IsPedCurrentWeaponSilenced(ped) and not name == `WEAPON_FIREEXTINGUISHER` and not name == `WEAPON_PETROLCAN` then
			Citizen.Wait(50)
			TriggerEvent("client:newStress",true,math.random(3))
			Citizen.Wait(800)
		elseif IsPedShooting(ped) and IsPedCurrentWeaponSilenced(ped) and not name == `WEAPON_FIREEXTINGUISHER` and not name == `WEAPON_PETROLCAN`  then 
			Citizen.Wait(50)
...
RegisterNetEvent("ab-hud:changeRange")
AddEventHandler("ab-hud:changeRange", function(pRange)
	if pRange == 1 then 
