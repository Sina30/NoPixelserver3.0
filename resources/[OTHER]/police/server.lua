...
AddEventHandler('police:targetCheckBank', function(target)
	local src = source
	local user = exports["ab-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
	...
	local target = exports["ab-base"]:getModule("Player"):GetUser(player)
	local character = target:getCurrentCharacter()
	local playerName = character.first_name .. ' ' .. character.last_name

	local pdunit = exports["ab-base"]:getModule("Player"):GetUser(src)
	if not pdunit:getVar("job") == "police" then
		local steamid, name = pdunit:getVar("name"), pdunit:getVar("steamid")

		exports["ab-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {target = playerName, reason = reason, cid = cid, time = tonumber(args[2]), src= src})
		DropPlayer(src, "")
	...
	exports["ab-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
...
AddEventHandler("police:deletecrimes", function(target)
	...
	if not pdunit:getVar("job" =="police") then
		...
		exports["ab-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {target = target , src = src})
		DropPlayer(src, "")
...
AddEventHandler('police:SeizeCash', function(target)
	...
	local userz = exports["ab-base"]:getModule("Player"):GetUser(target)
	...
	if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
		exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid",{['job'] = jobType, ["cid"] = arg}, function(result)
			if(result[1]) then
				...
				exports["ab-log"]:AddLog("HC Added Job","Character: "..char.id.." Changed Char: "..arg.." Job whitelisting: "..jobType.." Type: Added", {source = src, char = char.id})
	...
	if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
		exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid",{['job'] = jobType, ["cid"] = arg}, function(result)
			if(result[1]) then
				...
				exports["ab-log"]:AddLog("HC Removed Job","Character: "..char.id.." Changed Char: "..arg.." Job whitelisting: "..jobType.." Type: Removed", {source = src, char = char.id})
...
function jailUser(player,reason,cid,time,src)
	local jailer = exports["ab-base"]:getModule("Player"):GetUser(src)
	...
	if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
		...
		exports["ab-log"]:AddLog("Exploiter", jailer, "User attempted to jail while not on pd; Reason: " ..tostring(reason), {target = player, reason = reason, cid = cid, time = time})
		DropPlayer(src, "")
		return
	end
	...
	TriggerEvent("ab-police:playerJailed", character)
...
AddEventHandler("911", function(args)
	...
	exports["ab-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
	...
	local users = exports["ab-base"]:getModule("Player"):GetUsers()
...
AddEventHandler("911r", function(args)
	...
	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
	exports["ab-log"]:AddLog("911r Chat", user, caller .. ": " .. message)
	...
	exports["ab-log"]:AddLog("911r Chat", user, char.first_name .. " " .. char.last_name .. ": ".. message, {target = target})
end)
