...
Citizen.CreateThread(function()
	while true do
		...
		TriggerServerEvent("ab-base:updatecoords",coords.x,coords.y,coords.z)
		Wait(13000)
	end
end)
	...
	while true do 
		...
		if recentsearch == 0 then
			...
			if endDistance < 0.7 or endDistance2 < 0.7 then
				...
				currentWeapons = exports["ab-inventory"]:GetCurrentWeapons() 
...
AddEventHandler("police:rob", function()
	...
	if not exports["isPed"]:isPed("dead") then
		...
		local finished = exports["ab-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["ab-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["ab-taskbar"]:taskBar(2000,"Unracking Shotgun")
		  	if finished == 100 then
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			SetVehicleDoorOpen(targetVehicle, 0, 1, 1)
	  		loadAnimDict('anim@narcotics@trash')
			TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)
			local finished = exports["ab-taskbar"]:taskBar(2000,"Unracking Rifle")
		  	if finished == 100 then
...
AddEventHandler('police:woxy', function()
	...
	if vehFront > 0 then
		...
		local finished = exports["ab-taskbar"]:taskBar(4000,"Grabbing Scuba Gear")
	  	if finished == 100 then
	  		loadAnimDict('anim@narcotics@trash')
    		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)	  		
...
AddEventHandler('giveRadioToPed', function()
	...
	if (not exports["ab-inventory"]:hasEnoughOfItem("radio",1,false) ) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["ab-taskbar"]:taskBar(2500,"Changing to Swat")
	  		if finished == 100 then
				ChangeToSkinNoUpdatePolice("s_m_y_swat_01")		
			end
			ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["ab-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not isDead then
		finished = exports["ab-ui"]:taskBarSkill(1200,7)
	end
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ems" and myjob ~= "doctor" then
				local bandages = exports["ab-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("ems:stomachpump", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["ab-taskbar"]:taskBar(10000,"Inserting stomach pump 🤢", false, true)
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["ab-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh(ent)
	...
	local finished = exports["ab-taskbar"]:taskBar(1000,"Impounding",false,true)	
...
local function serviceVehicle(arg, livery, isEmsWhiteListed, cb)
	...
	Citizen.CreateThread(function()
		...
		TriggerEvent("ab-admin:runSpawnCommand",selectedSkin, livery)
	end)
end
...
if isCop then
	...
	if cmd == "fix" then
		local finished = exports["ab-taskbar"]:taskBar(3500,"Completing Task",false,true)	
