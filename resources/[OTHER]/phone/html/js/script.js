        ...
        switch (item.openSection) {
            ...
            case "callState":
                ...
                case "notify":
                    ...
                    $(".phone-shell").removeClass("slideout").addClass('slidein').show().css("bottom" , "-540px").add($('.phone-screen')).fadeIn(500);
                    setTimeout(() => {
                        $('#twatter-notification').addClass("slideinnotify").css({"buttom": "50px", "right": "51px"}).fadeIn()
                    ...
                    case "notifytext":
                        ...
                        $(".phone-shell").removeClass("slideout").addClass('slidein').show().css("bottom" , "-540px").add($('.phone-screen')).fadeIn(500);
                        setTimeout(() => {
                            $('#text-notification').addClass("slideinnotify").css({"buttom": "50px", "right": "51px"}).fadeIn()
                            $('.text-notification-title').text(item.handle);
                            $('.text-notitication-body').text(decodeEntities(item.message));
                            setTimeout(() => {
                                $('#text-notification').removeClass("slideinnotify").addClass("slideoutnotify").fadeOut()
                                $(".phone-shell").removeClass("slidein").addClass("slideout").fadeOut()
                                $('#text-notification').fadeOut();
                                noty = false
                            }, 5000);
                        }, 200);
                        ...
                        $(".phone-shell").removeClass("slideout").addClass('slidein').show().css("bottom" , "-540px").add($('.phone-screen')).fadeIn(500);
                        setTimeout(() => {
                            $('#email-notification').addClass("slideinnotify").css({"buttom": "50px", "right": "51px"}).fadeIn()
                            $('.email-notification-title').text(item.handle);
                            $('.email-notitication-body').text(decodeEntities(item.message));
                            setTimeout(() => {
                                $('#email-notification').removeClass("slideinnotify").addClass("slideoutnotify").fadeOut()
                                $(".phone-shell").removeClass("slidein").addClass("slideout").fadeOut()
...
function addEmails(emails) {
    for (let email of Object.keys(emails)) {
        ...
        <div class="row">
        <div class="col s12 center-align">
            <div class="card-panel" style="background: #31455E;border-radius: 10px;">
            ...
            <div class="collapsible-header" style="background-color: #31455E; color: white">
                ...
                <div class="collapsible-body garage-body" style="background-color: #31455E; color: white">
                    ...
                    <li class="collection-item" style="background-color: #31455E;">Name: ${stockEntry.name}</li>
                        <li class="collection-item" style="background-color: #31455E;">Shares: ${stockEntry.clientStockValue}</li>
                        <li class="collection-item" style="background-color: #31455E;">Float: ${stockEntry.available}</li>
                        <li class="collection-item" style="background-color: #31455E;">Value: ${stockEntry.value}</li>
            ...
            if (message && message.handle && message.message) {
                ...
                if (twat !== "") {
                  var twatEntry = $(`<div class="row no-padding">
                          <div class="col s12">
                              <div class="card blue darken-3 twat-card" style="background: rgb(20, 29, 39) !important;">
                                  <div class="card-content white-text twatter-content" style="background: #31455E; border-radius: 10px 10px 0 0;">
                                       <span class="card-title twatter-title" style="font-size: 12px !important; font-weight: 400 !important;">${message.handle}</span>
                                       <p>${twat}</p>
                                  </div>
                                  <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;background: #31455E;border-radius: 0 0 10px 10px;">
                                      <span data-poster="${message.handle}" class="twat-reply white-text"><i class="fas fa-reply fa-1x"></i></span>
                                      <span class="right white-text" aria-label="${moment.utc(message.timestamp).local().calendar(null, calendarFormatDate)}" data-balloon-pos="down">${moment.utc(message.timestamp).local().fromNow()}</span>
                                  </div>
                               </div>
                          </div>
                       </div>`);
    ...
    var element = $(`
        <li id="${nameID}-${item.number}">
            <div class="collapsible-header" style="background: #31455E; font-size:12px">
                ...
                <span class="new badge number-badge blue darken-1" style="width:40%" data-badge-caption="">${phoneNumber}</span>
            </div>
...
$("#twat-gifword").change(function() {
   ...
   $(document).on('click', '#twat-sendimg', function (e) {
    $.post('http://phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
