...
AddEventHandler("ab-jobmanager:playerBecameJob", function(job)
    if job == "trucker" then
        trucker = true
    end
end)
...
AddEventHandler("phone:error", function()
      SendNUIMessage({
        openSection = "error",
        textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using RedSkys Phone Services.",
      })   
end)
...
AddEventHandler("stocks:timedEvent", function(typeSent)
  local success = true
  local timer = 600000
  TriggerEvent("chatMessage", "EMAIL ", 8, "Yo, here are the coords for the drop off, you have 10 minutes - I already zoinked your Pixerium")
  refreshmail()
  local location = pickuppoints[math.random(#pickuppoints)]
  CreateBlip(location)
  while timer > 0 do
...
function hasDecrypt2()
    if exports["ab-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["ab-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["ab-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["ab-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["ab-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["ab-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["ab-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["ab-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      exports["ab-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
      exports["ab-inventory"]:hasEnoughOfItem("stolens8",1,false) or
      exports["ab-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
      exports["ab-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
      exports["ab-inventory"]:hasEnoughOfItem("assphone",1,false) or
      exports["ab-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if exports["ab-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
AddEventHandler('phone:addToCall', function(voipchannel)
  exports['ab-voice']:addPlayerToCall(tonumber(voipchannel))
end)
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['ab-voice']:removePlayerFromCall()
end)
...
AddEventHandler('phone:addnotification', function(name,message)
    ...
    SendNUIMessage({openSection = "notifyemail", handle = 'Email Received.', message = message})
...
AddEventHandler('phone:newSMS', function(id, number, message, mypn, date, recip)
  ...
  if hasPhone() then
    ...
    if phoneNotifications then
      ...
      SendNUIMessage({openSection = "notifytext", handle = number, message = message})
      PlaySound(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0, 0, 1)
      Wait(5000)
      closeGui()
    end
...
RegisterNUICallback('settings', function()
  local controls = exports["ab-base"]:getModule("DataControls"):getBindTable()
  local settings = exports["ab-base"]:getModule("SettingsData"):getSettingsTable()
...
RegisterNUICallback('settingsUpdateToko', function(data, cb)
  if data.tag == "settings" then
    exports["ab-base"]:getModule("SettingsData"):setSettingsTableGlobal(data.settings, true)
  elseif data.tag == "controlUpdate" then
    exports["ab-base"]:getModule("DataControls"):encodeSetBindTable(data.controls)
  end
end)

RegisterNUICallback('settingsResetToko', function()
  TriggerEvent("ab-base:cl:player_reset", "tokovoip")
end)

RegisterNUICallback('settingsResetControls', function()
  TriggerEvent("ab-base:cl:player_control", nil)
end)
...
RegisterNUICallback('pingContact', function(data, cb)
  TriggerServerEvent('pingPhoneNum', data)
end)
]]
local selfieMode = false
...
RegisterNUICallback('PostNewImage', function(data, cb)
  TriggerEvent('animation:sms', false)
        ...
        while takePhoto do
            Citizen.Wait(0)
            exports["ab-ui"]:showInteraction("[UP ARROW] SELFIE | [DOWN] CAMERA | [ENTER] CLICK", "classic")
            if IsControlJustPressed(1, 27) then -- Toogle Mode
                -- frontCam = not frontCam
                CellFrontCamActivate(true)
            elseif IsControlJustPressed(1, 173) then -- Toogle Mode
                CellFrontCamActivate(false)
            else if IsControlJustPressed(1, 176) then
                exports['screenshot-basic']:requestScreenshotUpload('https://discord.com/api/webhooks/871734833049657364/3KaksAuXTTgFkuUlaWBNodHAwBlHA6a1y0WiDSWrc1eKABVjQp8wjFpwUkh2vGB4ZX8Z', 'files[]', function(data2)
                  DestroyMobilePhone()
                  CellCamActivate(false, false)
